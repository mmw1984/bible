...
class BibleRepository {
  ...
  Future<List<ScriptureHit>> search(String query, {int limit = 80}) async {
    ...
    for (final book in bibleBooks) {
      final chinese = await _assetBook('cuv', book.id);
      final english = await _assetBook('web', book.id);
  ...
  Future<Map<String, List<String>>> _assetBook(
    ...
    try {
      final payload = await rootBundle.loadString(
        'assets/bible/$translation/$book.json',
      );
  ...
  Future<List<String>> _translation(
    ...
    try {
      final response = await http
          .get(
            Uri.parse('https://bible-api.com/data/$translation/$book/$chapter'),
          )
      ...
      // Retain partial bundled data as a fallback for future offline work.
    }
    return const [];
  }
}
